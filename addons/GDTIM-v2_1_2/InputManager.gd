...
var current_raw_gesture : RawGesture = RawGesture.new() # Current raw_gesture
...
func _handle_mouse_motion(event : InputEventMouseMotion) -> void:
	if current_raw_gesture.size() == 1 and _mouse_event == Gesture.SINGLE_DRAG:
		_emit("drag", _native_drag_event(0, event.position, event.relative, event.velocity))
	elif current_raw_gesture.size() == 2 and _mouse_event == Gesture.MULTI_DRAG:
		var offset = Vector2(5,5)
		var e0 = _native_drag_event(0, event.position-offset, event.relative, event.velocity)
		current_raw_gesture._update_screen_drag(e0)
		var e1 = _native_drag_event(1, event.position+offset, event.relative, event.velocity)
		current_raw_gesture._update_screen_drag(e1)
		_emit("multi_drag", InputEventMultiScreenDrag.new(current_raw_gesture,e0))
		_emit("multi_drag", InputEventMultiScreenDrag.new(current_raw_gesture,e1))
...
func _handle_screen_touch(event : InputEventScreenTouch) -> void:
	...
	if !event.pressed and not event.index in current_raw_gesture.presses:
		return

	current_raw_gesture._update_screen_touch(event)
	_emit("raw_gesture", current_raw_gesture)
	var index : int = event.index
	if event.pressed:
		if current_raw_gesture.size() == 1: # First and only touch
			_long_press_timer.start(LONG_PRESS_TIME_THRESHOLD)
			_single_touch_cancelled = false
			_emit("single_touch", InputEventSingleScreenTouch.new(current_raw_gesture))
		elif !_single_touch_cancelled :
				_single_touch_cancelled = true
				_cancel_single_drag()
				_emit("single_touch", InputEventSingleScreenTouch.new(current_raw_gesture))
	else:
		var fingers : int = current_raw_gesture.size() 
		if index == 0:
			_emit("single_touch", InputEventSingleScreenTouch.new(current_raw_gesture))
			if !_single_touch_cancelled:
				var distance : float = (current_raw_gesture.releases[0].position - current_raw_gesture.presses[0].position).length()
				if current_raw_gesture.elapsed_time < TAP_TIME_LIMIT and distance <= TAP_DISTANCE_LIMIT:
					_emit("single_tap", InputEventSingleScreenTap.new(current_raw_gesture))
				if current_raw_gesture.elapsed_time < SWIPE_TIME_LIMIT and distance > SWIPE_DISTANCE_THRESHOLD:
					_emit("single_swipe", InputEventSingleScreenSwipe.new(current_raw_gesture))
		if current_raw_gesture.active_touches == 0: # last finger released
			if _single_touch_cancelled:
				var distance : float = (current_raw_gesture.centroid("releases","position") - current_raw_gesture.centroid("presses","position")).length()
				if current_raw_gesture.elapsed_time < TAP_TIME_LIMIT and distance <= TAP_DISTANCE_LIMIT and\
					 current_raw_gesture.is_consistent(TAP_DISTANCE_LIMIT, FINGER_SIZE*fingers) and\
					 _released_together(current_raw_gesture, MULTI_FINGER_RELEASE_THRESHOLD):
					_emit("multi_tap", InputEventMultiScreenTap.new(current_raw_gesture))
				if current_raw_gesture.elapsed_time < SWIPE_TIME_LIMIT and distance > SWIPE_DISTANCE_THRESHOLD and\
					 current_raw_gesture.is_consistent(FINGER_SIZE, FINGER_SIZE*fingers) and\
					 _released_together(current_raw_gesture, MULTI_FINGER_RELEASE_THRESHOLD):
					_emit("multi_swipe", InputEventMultiScreenSwipe.new(current_raw_gesture))
			_end_gesture()
		_cancel_single_drag()


func _handle_screen_drag(event : InputEventScreenDrag) -> void:
	current_raw_gesture._update_screen_drag(event)
	_emit("raw_gesture", current_raw_gesture)
	if current_raw_gesture.drags.size() > 1:
		_cancel_single_drag()
		var gesture : int = _identify_gesture(current_raw_gesture)
		if gesture == Gesture.PINCH:
			_emit("pinch", InputEventScreenPinch.new(current_raw_gesture, event))
		elif gesture == Gesture.MULTI_DRAG:
			_emit("multi_drag", InputEventMultiScreenDrag.new(current_raw_gesture, event))
		elif gesture == Gesture.TWIST:
			_emit("twist",InputEventScreenTwist.new(current_raw_gesture, event))
	else:
		if _single_drag_enabled:
			_emit("single_drag", InputEventSingleScreenDrag.new(current_raw_gesture))
		else:
...
func _on_drag_startup_timer_timeout() -> void:
	_single_drag_enabled = current_raw_gesture.drags.size() == 1


func _on_long_press_timer_timeout() -> void:
	var ends_centroid   : Vector2    = Util.centroid(current_raw_gesture.get_ends().values())
	var starts_centroid : Vector2    = current_raw_gesture.centroid("presses", "position")
	var distance        : float      = (ends_centroid - starts_centroid).length()

	if current_raw_gesture.releases.is_empty() and distance <= LONG_PRESS_DISTANCE_LIMIT and\
		 current_raw_gesture.is_consistent(LONG_PRESS_DISTANCE_LIMIT, FINGER_SIZE*current_raw_gesture.size()):
		if _single_touch_cancelled:
			_emit("multi_long_press", InputEventMultiScreenLongPress.new(current_raw_gesture))
		else:
			_emit("single_long_press", InputEventSingleScreenLongPress.new(current_raw_gesture))


func _end_gesture() -> void:
	_single_drag_enabled = false
	_long_press_timer.stop()
	current_raw_gesture = RawGesture.new()
...
func _add_timer(timer : Timer, func_name : Callable) -> void:
	timer.one_shot = true
	if func_name:
